...
const raiz = numbers.map(valor => Math.sqrt(valor));
...
const funcao = (array) =>  array.map(dado => dado * 10 );

numbers = funcao(numbers);
...
const filtraIdade = (array) =>  array.filter(idade => idade >= 18);

const permitido = filtraIdade(ages)
...
const idade = dogs.reduce((acc, net) => acc + net.age, 0)//reutilizei o array ali de cima, mas da pra unir das duas coisas em uma função só
